...
class TONCenterAdapter(BaseAdapter):
    ...
    async def get_transactions(self, address: str, limit: int = 10) -> List[PaymentInfo]:
    ...
    def _parse_ton_tx(self, tx: dict, address: str = None) -> Optional[PaymentInfo]:
        ...
        try:
            ...
            return PaymentInfo(
                ...
                block_height=None,
                confirmations=1,
                fee=Decimal(tx.get("fee", "0")) / Decimal(10 ** self.network_config.decimals),
                from_address=source,
...
class TRONGridAdapter(BaseAdapter):
    ...
    def _parse_tron_tx(self, tx: dict, address: str = None) -> Optional[PaymentInfo]:
        ...
        try:
            ...
            amount = Decimal(value_data.get("amount", 0)) / Decimal(10 ** self.network_config.decimals)
            ...
            return PaymentInfo(
                ...
                amount=amount,
                currency=self.network_config.symbol,
                status=PaymentStatus.CONFIRMED if tx.get("ret", [{}])[0].get("contractRet") == "SUCCESS" else PaymentStatus.FAILED,
...
def get_adapter(adapter_name: str, rpc_url: str, network_config) -> Optional[BaseAdapter]:
